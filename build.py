...
@media(max-width:680px){
  ...
  /* filter: thanh cuộn ngang 1 hàng + fade mép + nút bung lưới */
  .chips-wrap{margin-bottom:18px;margin-left:-12px;margin-right:-12px}
  .chips-lbl{padding:0 12px;display:flex;align-items:center;justify-content:space-between}
  .chips-lbl .toggle{font-size:11.5px;font-weight:700;color:var(--accent);background:#eef2ff;border:0;border-radius:14px;padding:5px 11px;cursor:pointer;display:inline-flex;align-items:center;gap:4px;text-transform:none;letter-spacing:0}
  .chips-scroller{position:relative}
  .chips-scroller::after{content:"";position:absolute;top:0;right:0;bottom:8px;width:42px;pointer-events:none;background:linear-gradient(90deg,rgba(247,249,250,0),var(--bg));opacity:1;transition:opacity .2s}
  .chips-scroller.at-end::after{opacity:0}
  ...
  /* khi bung: lưới 2 hàng đầy đủ, không cuộn */
  .chips-wrap.expanded .chips{flex-wrap:wrap;overflow:visible;padding-bottom:2px}
  .chips-wrap.expanded .chips-scroller::after{display:none}
...
function renderGrid(){
  ...
  document.getElementById("grid").innerHTML=list.map(f=>`
    <div class="card" onclick="openDetail('${f.slug}')">
      <div class="thumb">
        ${f.done?'':'<div class="badge todo">soon</div>'}
...
function toggleChips(){
  const w=document.getElementById("chipsWrap"), b=document.getElementById("chipsToggle");
